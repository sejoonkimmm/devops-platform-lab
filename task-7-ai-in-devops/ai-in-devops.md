...
That taught me which failure to watch for. AI does not fail loudly; it produces something that looks right. In application code a wrong guess usually throws. In infrastructure config it can look right, pass review, and fail later without a sound, the same shape as the silent Kafka bug in Task 1.
...
Over the next year I want to push the harness idea past config, into agents that cross-check incident response, read my Terraform before it plans, and catch the answer that looks right but is wrong, before I see it. I also want to find where that stops being worth it, because at some point a check that is itself AI needs its own check. The job is not disappearing into the tools; it is moving into deciding what to trust.
